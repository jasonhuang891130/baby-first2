...
export function Chatbot({ onCreatePlan }: ChatbotProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Get only the most recent and relevant messages for context
  ...
  return (
    <div className="fixed bottom-0 right-0 z-50 md:bottom-8 md:right-8">
      {isOpen ? (
        <div 
          className="bg-white flex flex-col w-full h-[100vh] md:h-[70vh] md:w-[550px] md:rounded-xl md:shadow-xl md:max-h-[calc(100vh-80px)] md:max-w-[calc(100vw-64px)]"
        >
          <div className="bg-yellow-500 text-black px-4 md:px-8 py-4 md:py-5 md:rounded-t-xl">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="font-semibold text-lg md:text-xl mb-1">Baby Nutrition Assistant</h3>
        ...
        <div className="relative group p-4 md:p-0">
          {showLabel && (
            <div className="hidden md:block absolute bottom-full right-0 mb-2 bg-yellow-500 text-black px-4 py-2 rounded-lg text-sm whitespace-nowrap animate-bounce shadow-lg">
          ...
          <button
            ...
            className="bg-yellow-500 text-black p-3 md:p-4 rounded-full shadow-xl hover:bg-yellow-400 hover:shadow-2xl transition-all duration-300 flex items-center gap-2 w-full md:w-auto justify-center"
          >
            <MessageCircle size={24} className="animate-pulse" />
            <span className="md:hidden group-hover:inline font-semibold">Baby Food Help</span>
          </button>
        </div>
      )}
    </div>
